...
密码重置使用邮箱验证码流程：用户在登录页输入注册邮箱，收到邮件中的验证码后在当前页面填写验证码和新密码，不需要点击邮件链接跳转。验证码长度以 Supabase Auth 项目配置为准，前端当前支持 6 到 10 位数字验证码。请在 Supabase Dashboard -> Authentication -> Email Templates -> Reset Password 中把模板改成包含 `{{ .Token }}` 的验证码邮件，例如：

```html
<h2>重置你的 CloseCircle 密码</h2>
<p>你的验证码是：</p>
...
Supabase Auth 的 Site URL 和 Redirect URLs 也需要同步填写线上域名，供注册确认、邀请链接等邮件流程使用。当前密码重置主流程使用验证码，不依赖用户点击重置链接。
